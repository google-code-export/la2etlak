module AutoCompleteMacrosHelper      
  ...
  #this method is used for getting a filtered result for the autocomplete
  def auto_complete_result(entries, phrase = nil)
    return unless entries
    items = entries.map { |entry| raw(content_tag("li", if (entry.kind_of?(User) ||entry.kind_of?(Interest)) then h(entry.send(:name)) else  h(entry.send(:title)) end )) }
  ...
  def text_field_with_auto_complete(object, method, tag_options = {}, completion_options = {})
    ...
    auto_complete_field("#{tag_id}", { :url => {:controller =>"autocomplete" , :action => "auto_complete_for_#{object}_#{method}" } }.update(completion_options))
  end
