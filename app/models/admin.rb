...
=end  
  def self.get_feed
   item1 =  Interest.desc(:created_at).where(:created_at.gte => (Date.today - 30))
   item2 =  Story.desc(:created_at).where(:created_at.gte => (Date.today - 30))
   item = item1+item2
...
#use as find_by_email 
  def self.get_admin_by_email(emailo)
